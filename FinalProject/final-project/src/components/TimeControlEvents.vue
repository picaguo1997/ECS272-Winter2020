...
export default {
  ...
  methods: {
    init() {
      ...
      const x1 = this.getDates(
        this.getNewsTimestamps(this.data_news),
        "x1",
        new Date("2020/01/01")
      );
      const x2 = this.getDates(
        this.getInfectionTimestamps(this.data_confirmed),
        "x2",
        new Date("2020/01/01")
      );

      const y_news = this.getNewsDataPoints(
        this.data_news,
        "news",
        new Date("2020/01/01")
      );
      ...
      this.chart = c3.generate({
        ...
        data: {
          ...
          columns: [x1, x2, y_confirmed, y_deaths, y_recovered, y_news],
          types: {
            news: "bar",
            confirmed: "spline",
            deaths: "spline",
